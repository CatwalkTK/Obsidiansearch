...
8.  **独立質問処理システム**: 各質問の処理において、会話履歴の適切な管理を実行します：
...
9.  **回答の表示**: AIの応答がチャットインターフェースに表示されます。
...
6. **承認機能**: 社内ナレッジで回答が見つからない場合、承認プロンプトが表示されます
...
#### テスト方法
承認機能をテストするには、Vault内に存在しない情報について質問してください：
- 「今日の天気は？」
- 「ChatGPTについて教えて」  
...
-   **ビルドツール**: Vite（高速HMR）
-   **AI技術**: 
    - Google Gemini API (`@google/genai`) - 同義語生成・検索
    - OpenAI API (`openai`) - GPT-4による高精度同義語生成
    - ベクトル埋め込み検索（コサイン類似度）
