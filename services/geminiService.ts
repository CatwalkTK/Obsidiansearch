...
export async function getGeminiAnswer(apiKey: string, context: string, conversationHistory: Message[]): Promise<string> {
  ...
  const systemInstruction = `あなたは、Obsidian Vaultのコンテンツから情報を抽出する、ルールベースのAIアシスタントです。あなたの仕事は、提供されたコンテキスト（テキストの断片）に*厳密に*基づいて、ユーザーの質問に答えることです。自由な解釈や推測はせず、以下のルールに厳密に従ってください。
